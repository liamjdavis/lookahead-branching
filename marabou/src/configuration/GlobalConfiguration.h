...
public:
    ...
    */
    ...
    /* Upper bound on the preselect pool for lookahead branching: at most this
       many constraints are scored by the cheap polarity heuristic before any
       lookahead evaluation is performed.
    */
    static const unsigned NUM_LOOKAHEAD_BRANCHES;

    /* Of the preselected pool, only the top NUM_LOOKAHEAD_CANDIDATES are
       evaluated using the expensive lookahead procedure that estimates phase
       fixes from trial splits.
    */
    static const unsigned NUM_LOOKAHEAD_CANDIDATES;

    /* Lookahead branching is only invoked when the SMT stack depth is strictly
       below this value, restricting it to the top of the search tree where the
       overhead is amortized over a large subtree.
    */
    static const unsigned LOOKAHEAD_MAX_STACK_DEPTH;
