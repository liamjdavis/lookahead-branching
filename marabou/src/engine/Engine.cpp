...
{
    // Lookahead branching relies on topological constraint information from
    // the network-level reasoner; without it we fall back to the default
    // branching heuristic.
    if ( !_networkLevelReasoner )
        return nullptr;
    ...
    // Preselect ReLUs based on polarity. Build a score map capped at
    // NUM_LOOKAHEAD_BRANCHES candidates, then keep the top
    // NUM_LOOKAHEAD_CANDIDATES for the more expensive lookahead evaluation below.
    ...
    for ( auto &plConstraint : constraints )
    {
             ...
             !plConstraint->phaseFixed() )
        {
            ...
            if ( scoreToConstraint.size() >= GlobalConfiguration::NUM_LOOKAHEAD_BRANCHES )
                break;
        }
    }
    List<PiecewiseLinearConstraint *> preselectedConstraints;

    unsigned numToSelect = std::min( GlobalConfiguration::NUM_LOOKAHEAD_CANDIDATES,
                                     (unsigned)scoreToConstraint.size() );
    ...
    for ( auto &plConstraint : preselectedConstraints )
    {
        if ( plConstraint->isActive() && !plConstraint->phaseFixed() )
        {
            ...
            // Recursively explore both polarities up to MAX_LOOKAHEAD_DEPTH and
            // collect, for each leaf of that exploration, the set of phases
            // that became fixed by side propagation.
            ...
            // The intersection of the fix sets across all leaves yields phases
            // that must hold regardless of how this constraint is decided, so
            // they can be applied unconditionally before branching.
            ...
            // Score this constraint by the ratio of the product to the sum of
            // its leaf phase-fix counts. This rewards constraints whose splits
            // each produce many fixes (large product) over those with a single
            // dominant split (large sum, small product).
    ...
    // Apply every phase fix shared across all explored leaves, then propagate
    // their effects until no further bound tightenings or case splits remain.
    ...
    int depth )
{
    ...
    // Propagate effects. We deliberately skip applyAllValidConstraintCaseSplits
    // here: it is expensive and the additional case splits it produces are not
    // needed to estimate phase fixes at this lookahead leaf.
    _boundManager.propagateTightenings();
    performSymbolicBoundTightening();
    applyAllBoundTightenings();

    // If we haven't reached max depth, branch recursively using the normal
    // heuristic to pick the next constraint to split on.
    ...
    {
        // Leaf of the lookahead tree: record how many phases this split
        // sequence forced. The +1 in the product keeps it nonzero when a leaf
        // produces no fixes, so other leaves can still influence the score.
        Map<PiecewiseLinearConstraint *, PhaseStatus> fixes;
        unsigned phaseFixes = countPhaseFixed( fixes );
        phaseFixedSum += phaseFixes;
        phaseFixedProduct *= ( phaseFixes + 1 );
...
PiecewiseLinearConstraint *Engine::pickSplitPLConstraint( DivideStrategy strategy )
{
    ...
    // Apply lookahead branching only near the top of the search tree, where its
    // overhead is amortized by the size of the subtree it prunes.
    if ( ( !_smtCore.inLookaheadMode() ) &&
         Options::get()->getBool( Options::USE_LOOKAHEAD_BRANCHING ) &&
         _smtCore.getStackDepth() < GlobalConfiguration::LOOKAHEAD_MAX_STACK_DEPTH )
    {
